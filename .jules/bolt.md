...
## 2026-02-14 - Missing Indexes on Appointments and Patients
**Learning:** Found N+1 query bottlenecks when fetching appointments by patient and patients by caregiver due to missing database indexes on foreign keys. Verified using `EXPLAIN QUERY PLAN` which showed `SCAN` instead of `SEARCH`.
**Action:** Added `index=True` to `AppointmentModel.patient_id` and `PatientModel.caregiver_id`. Verified query plan change from `SCAN` to `SEARCH USING INDEX`.
